	        ...
	        <div class="col-6 col-md-3">
	            <a href="{{ route('complaint.super-categories') }}">
	                <div class="counter-card purple d-flex align-items-center justify-content-between category-box">
	                    <div class="header">@php Session::put('service_building_id', $servicebuilding->id); @endphp</div>
	                    <div class="body">
<!-- 	                        <img src="{{ \Illuminate\Support\Facades\Storage::url('category-icons/' . str_replace('/', '', $servicebuilding->name) . '/' . $servicebuilding->icons) }}" alt="{{ $servicebuilding->name }}"> -->
	                        <span class="head"><h3>{{ $servicebuilding->name }}</h3></span>
	                    </div>
	                </div>
	            </a>
	        </div>
