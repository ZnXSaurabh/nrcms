	...
	<div class="row mb-3">
		@foreach($locations as $location)
	        <div class="col-6 col-md-3">
	            <a href="{{ route('complaint.building', $location->location_id) }}">
	                <div class="counter-card purple d-flex align-items-center justify-content-between category-box">
	                    <div class="header"></div>
	                    <div class="body">	            	
<!-- 	                        <img src="{{ \Illuminate\Support\Facades\Storage::url('category-icons/' . str_replace('/', '', $location->name) . '/' . $location->icons) }}" alt="{{ $location->name }}"> -->
	                        <span class="head"><h3>{{ $location->location->name }}</h3></span>
	                    </div>
	                </div>
	            </a>
	        </div>
